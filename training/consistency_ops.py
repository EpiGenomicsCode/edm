...
def heun_hop_edm_stochastic(
    ...
    *,
    ...
    """
    Stochastic Heun hop (teacher) with Karras-style churn (EDM PF-ODE):
        1) Optionally add noise: sigma_t_hat = sigma_t * (1 + gamma), x_t_hat = x_t + noise
        2) Run Heun from (x_t_hat, sigma_t_hat) to sigma_s

    This mirrors Algorithm 2 in Karras et al. (EDM) with defaults tuned for ImageNet:
        S_churn=40, S_min=0.05, S_max=50.0, S_noise=1.003 (see training_loop.py)
    """
    assert isinstance(x_t, torch.Tensor)
    out_dtype = x_t.dtype
    x32 = x_t.to(torch.float32)

    # Round sigmas and broadcast.
    sigma_t_r = net.round_sigma(torch.as_tensor(sigma_t, device=x32.device))
    sigma_s_r = net.round_sigma(torch.as_tensor(sigma_s, device=x32.device))
    sigma_t_b = _expand_sigma_to_bchw(sigma_t_r, x32)
    sigma_s_b = _expand_sigma_to_bchw(sigma_s_r, x32)
    ...
    if torch.any(sigma_t_b == 0) or torch.any(sigma_s_b == 0):
        raise ValueError("heun_hop_edm_stochastic received σ=0. Avoid σ=0 in teacher hops.")

    # Churn strength per-step.
    if num_steps is None or num_steps <= 0 or S_churn <= 0:
        gamma_base = 0.0
    else:
        gamma_base = min(S_churn / float(num_steps), math.sqrt(2) - 1.0)

    # Enable churn only within [S_min, S_max].
    sigma_t_scalar = torch.as_tensor(sigma_t_r, device=x32.device, dtype=torch.float32)
    churn_mask = (sigma_t_scalar >= float(S_min)) & (sigma_t_scalar <= float(S_max))
    gamma = torch.where(churn_mask, torch.as_tensor(gamma_base, device=x32.device, dtype=torch.float32), torch.zeros_like(sigma_t_scalar))

    # Boosted sigma_t and noise injection.
    sigma_t_hat = net.round_sigma(sigma_t_scalar * (1.0 + gamma))
    sigma_t_hat_b = _expand_sigma_to_bchw(sigma_t_hat, x32)
    noise_mag = torch.clamp(sigma_t_hat_b ** 2 - sigma_t_b ** 2, min=0.0).sqrt() * float(S_noise)
    if torch.any(noise_mag != 0):
        x_t_hat = x32 + noise_mag * torch.randn_like(x32)
    else:
        x_t_hat = x32

    # Heun from (x_t_hat, sigma_t_hat) -> sigma_s.
    denoised_t = net(x_t_hat, sigma_t_hat, class_labels=class_labels, augment_labels=augment_labels).to(torch.float32)
    k1 = (x_t_hat - denoised_t) / sigma_t_hat_b

    x_eul = x_t_hat + (sigma_s_b - sigma_t_hat_b) * k1
    denoised_s = net(x_eul, sigma_s_r, class_labels=class_labels, augment_labels=augment_labels).to(torch.float32)
    k2 = (x_eul - denoised_s) / sigma_s_b

    x_s = x_t_hat + 0.5 * (sigma_s_b - sigma_t_hat_b) * (k1 + k2)
    return x_s.to(out_dtype)
